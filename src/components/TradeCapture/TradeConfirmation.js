...
const TradeConfirmation = () => {
  ...
  const buildReportRows = (type) => {
    const groups = type === 'sale'
      ? Object.entries(groupedData.sales)
      : Object.entries(groupedData.purchases);
  ...
  const exportTradeReportToExcel = () => {
    const salesRows = buildReportRows('sale');
    const purchaseRows = buildReportRows('purchase');
    ...
    XLSX.writeFile(wb, `trade-report-${safeDate}.xlsx`);
  };

  const exportTradeReportToPdf = () => {
    ...
    cursorY += 14;
    doc.text('We wish to inform you that the following transaction(s) were done on the above date.', marginX, cursorY);
    cursorY += 18;

    const head = [[
      'Trade Date', 'Contract No', 'No of Shares', 'Price/Avg', 'Gross Amount', 'Brokerage',
      'SEC', 'Exchange', 'CDS', 'GOV CESS', 'Clearing Fees', 'Net Amount', 'Settlement', 'Foreign Brokerage'
    ]];
    ...
    const safeDate = (reportDate || 'trade-report').replace(/[^\w.-]+/g, '_');
    doc.save(`trade-report-${safeDate}.pdf`);
  };
          ...
          {activeTab === 'trade-report' ? (
            <div className="tc-trade-report-export">
              <ExportPdfExcelButtons
                ...
                onExportExcel={exportTradeReportToExcel}
                onExportPdf={exportTradeReportToPdf}
              />
            </div>
          ) : null}
